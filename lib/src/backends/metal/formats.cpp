...
{
	bool MetalGetTextureInfo(void * impl, const TextureHandle texture, TextureInfo * out, Error * error) noexcept
	{
		AZO_RHI_PROFILE_ZONE("rhi.metal.getTextureInfo");
		...
		const MetalTextureSlot * const slot = device->textures.Resolve(texture, false);
		if (slot == nullptr)
		{
	...
	bool MetalGetBufferInfo(void * impl, const BufferHandle buffer, BufferInfo * out, Error * error) noexcept
	{
		AZO_RHI_PROFILE_ZONE("rhi.metal.getBufferInfo");
		auto * device = static_cast<MetalDevice *>(impl);
		if (out == nullptr)
		{
		...
		const MetalBufferSlot * const slot = device->buffers.Resolve(buffer, false);
		if (slot == nullptr)
		{
