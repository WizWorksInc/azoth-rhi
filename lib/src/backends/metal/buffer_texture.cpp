...
namespace azo::rhi::metal
{
	BufferHandle MetalCreateBuffer(void * impl, const BufferDesc & desc, Error * error) noexcept
	{
		...
		if (!MetalRefuseUnexportable(desc.exportableHandleTypes, {}, "Metal exports no buffers, so a buffer cannot be created exportable", error))
		{
			return BufferHandle{};
		}
		...
		// Resident from here, so a set that names it after the list closed still reaches it.
		device->NoteAllocation(MetalDevice::Residency::eBuffers, buffer.get());
	...
	TextureHandle MetalCreateTexture(void * impl, const TextureDesc & desc, Error * error) noexcept
	{
		...
		device->NoteAllocation(MetalDevice::Residency::eTextures, texture.get());
